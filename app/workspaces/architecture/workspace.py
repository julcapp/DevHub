...
class ArchitectureWorkspace(QWidget):
    """Центр архитектуры: сводка, интерактивная карта, риски и предупреждения."""
    def __init__(self)->None:
        ...
        self.dependencies=QListWidget(); self.warnings=QListWidget(); self.top_risks=QListWidget(); self.modules=QListWidget(); self.module_details=QTextEdit(); self.module_details.setReadOnly(True); self.graph_view=ArchitectureGraphView(); self.metrics:dict[str,QLabel]={}; self._build_ui()
    def _build_ui(self)->None:
        ...
        risk_box=QWidget(); risk_layout=QVBoxLayout(risk_box); risk_layout.setContentsMargins(0,0,0,0); risk_layout.addWidget(QLabel("Top Risks — приоритет проверки")); risk_layout.addWidget(self.top_risks); layout.addWidget(risk_box,1)
    ...
    def _show_summary(self,summary:ArchitectureSummary)->None:
        ...
        self.dependencies.clear(); self.modules.clear(); self.module_details.clear(); self.warnings.clear(); self.top_risks.clear()
        self.dependencies.addItems(summary.dependencies or ("Внутренние зависимости пока не обнаружены",)); self.modules.addItems([f"{x.name}  [{x.risk_level}: {x.risk_score}]" for x in summary.module_details]); self.warnings.addItems(summary.warnings or ("Критических архитектурных предупреждений не обнаружено",)); self.top_risks.addItems([f"#{i+1} {x.name} — {x.risk_level}, {x.risk_score}/100, связанность {x.coupling}" for i,x in enumerate(summary.top_risks)] or ["Риски не обнаружены"]); self.graph_view.show_summary(summary)
    ...
    def _select_risk_row(self,row:int)->None:
        if self.summary is None or row<0 or row>=len(self.summary.top_risks):return
