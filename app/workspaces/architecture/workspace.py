...
class ArchitectureWorkspace(QWidget):
    """Центр архитектуры: сводка, модули, зависимости и предупреждения."""
    ...
    def _build_ui(self) -> None:
        ...
        grid = QGridLayout()
        names = [
            ("files", "Файлы"), ("folders", "Папки"), ("modules", "Модули"),
            ("classes", "Классы"), ("methods", "Методы"), ("functions", "Функции"),
            ("imports", "Импорты"), ("internal_dependencies", "Внутренние зависимости"),
            ("nodes_total", "Узлы графа"), ("edges_total", "Связи графа"),
        ]
        for index, (key, text) in enumerate(names):
            ...
            grid.addWidget(QLabel(text), row, column)
        ...
        dep_layout.addWidget(QLabel("Все зависимости модулей"))
        dep_layout.addWidget(self.dependencies)

        warn_box = QWidget()
        warn_layout = QVBoxLayout(warn_box)
        warn_layout.setContentsMargins(0, 0, 0, 0)
        warn_layout.addWidget(QLabel("Архитектурные предупреждения"))
        warn_layout.addWidget(self.warnings)

        bottom.addWidget(dep_box)
        bottom.addWidget(warn_box)
    ...
    def _show_summary(self, summary: ArchitectureSummary) -> None:
        ...
        if summary.warnings:
            self.warnings.addItems(summary.warnings)
        else:
            self.warnings.addItem("Критических архитектурных предупреждений не обнаружено")
        ...
        self.status_label.setText(
            f"Анализ завершён: {summary.nodes_total} узлов, {summary.edges_total} связей{cycle_text}."
        )
